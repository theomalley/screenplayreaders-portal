...
Route::post('/incoming-assignment', [IncomingAssignmentController::class, 'store'])->middleware('throttle:30,1');

// PORTAL INTEGRATION: called by WordPress sr-upload-system.php to populate the reader dropdown
Route::get('/readers', [ReadersController::class, 'index'])->middleware('throttle:60,1');

// PORTAL INTEGRATION: called by WordPress sr-upload-system.php to fetch block-reader limit settings
Route::get('/upload-settings', [UploadSettingsController::class, 'index'])->middleware('throttle:60,1');

// ZAPIER INTEGRATION: called by sr-orders zap after HelpScout ticket is created — stores order → conversation ID
Route::post('/helpscout-conversation', [HelpScoutConversationController::class, 'store'])->middleware('throttle:30,1');
...
Route::post('/order-revenue', [OrderRevenueController::class, 'store'])->middleware('throttle:30,1');
...
Route::post('/budget-order', [\App\Http\Controllers\Api\BudgetWebhookController::class, 'store'])->middleware('throttle:30,1');
...
Route::post('/script-registration', [\App\Http\Controllers\Api\ScriptRegistrationWebhookController::class, 'store'])->middleware('throttle:30,1');
