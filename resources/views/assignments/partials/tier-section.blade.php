{{-- Shared by assignments/index.blade.php's admin view — one instance per dynamic tier (plus
     ...
     $sectionKey (unique per section — tier id, or 'unassigned'). Expanded/collapsed state persists
     per browser via localStorage (Alpine's collapsibleSection component, index.blade.php). --}}
@php $isOnboarding = $isOnboarding ?? false; @endphp
<div class="mb-6" x-data="collapsibleSection('{{ $sectionKey }}')">
    <button type="button" @click="toggle()"
            class="flex items-center gap-1.5 mb-2 px-1 group cursor-pointer">
        <svg class="w-3 h-3 text-gray-400 transition-transform duration-150" :class="open ? 'rotate-90' : ''"
             fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
        </svg>
        <h3 class="text-xs font-semibold {{ $isOnboarding ? 'text-amber-500' : 'text-gray-400' }} uppercase tracking-wider group-hover:text-indigo-600">
            {{ $sectionTitle }} <span class="text-gray-300 font-normal">({{ $sectionAssignments->count() }})</span>
        </h3>
    </button>
