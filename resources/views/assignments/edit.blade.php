        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5 items-start"
                ...
                <div class="space-y-5 min-w-0">
                    ...
                    <form id="update-form" method="POST" action="{{ route('assignments.update', $assignment) }}">
                        @csrf
                        @method('PATCH')

                        {{-- Assignment Rules --}}
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 space-y-5">
                            <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Assignment Rules</h3>
                            ...
                            <div>
                                ...
                                @php $selectedTierIds = $v('tiers', $assignment->tiers->pluck('id')->all()) ?? []; @endphp
                                <div class="flex flex-wrap gap-x-5 gap-y-2 mt-1 p-3 bg-gray-50 border border-gray-200 rounded-md">
                            ...
                            <div>
                                ...
                                <div class="mt-1 p-3 bg-gray-50 border border-gray-200 rounded-md space-y-2">
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 mt-5">
                            ...
                            <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
                                @php
                                    $hiddenFromReaderIds = $isOld
                                        ? (array) old('hidden_from_reader_ids', [])
                                        : array_map('strval', $assignment->hidden_from_reader_ids ?? []);
                                @endphp
                                @foreach ($readers as $reader)
                                    <label class="flex items-center gap-2 cursor-pointer select-none text-sm text-gray-700">
                                        <input type="checkbox" name="hidden_from_reader_ids[]" value="{{ $reader->id }}"
                                            {{ in_array((string) $reader->id, $hiddenFromReaderIds, true) ? 'checked' : '' }}
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 focus:ring-offset-0" />
                                        {{ $reader->readerProfile?->initials ?? $reader->name }}
                                    </label>
                                @endforeach
                        ...
                        {{-- Advanced Overrides — collapsed by default, rarely used --}}
                        <details class="bg-white rounded-lg shadow-sm border border-gray-200 mt-5">
                            <summary class="p-5 cursor-pointer select-none list-none flex items-center justify-between">
                                <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Advanced Overrides</h3>
                                <span class="text-gray-400 text-sm">＋</span>
                            </summary>
                            <div class="px-5 pb-5 space-y-3">
                                ...
                                <label class="flex items-start gap-3 cursor-pointer">
                                    ...
                                    <input type="checkbox" id="exempt_from_capacity" name="exempt_from_capacity" value="1"
                                        {{ old('exempt_from_capacity', $assignment->exempt_from_capacity ?? false) ? 'checked' : '' }}
                                        class="mt-0.5 rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" />
                                    <span class="text-sm text-gray-700">
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 mt-5"
                            ...
                            <h3 class="text-xs font-semibold text-blue-700 uppercase tracking-wider mb-2">
                                Notes
                                <span class="ml-1 text-[10px] font-normal text-blue-400 normal-case tracking-normal">(visible to the reader)</span>
                            </h3>
                    ...
                    <div id="internal-notes" class="bg-white rounded-lg shadow-sm border border-amber-200 p-5">
                ...
                {{-- ============================ SIDEBAR ============================ --}}
                <div class="space-y-5 order-first lg:order-none lg:sticky lg:top-5">

                    {{-- Actions --}}
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 space-y-3">
                        <div class="flex items-center gap-2">
                            <x-primary-button form="update-form" class="flex-1 justify-center">Save Changes</x-primary-button>
                            <a href="{{ route('assignments.index') }}"
                               class="text-sm text-gray-500 hover:text-gray-700 px-2 whitespace-nowrap">Cancel</a>
                        </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="order_number" name="order_number" form="update-form"
                        ...
                        <div>
                            ...
                            <select id="status" name="status" form="update-form"
                                x-model="statusVal"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                        ...
                        <div x-show="statusVal === 'cancelled'" x-cloak>
                            ...
                            <textarea id="cancellation_reason" name="cancellation_reason" form="update-form" rows="2"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex gap-2">
                                <input type="date" id="available_at_date" form="update-form" x-model="availableAtDate"
                                    class="block w-1/2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" />
                                <select id="available_at_time" form="update-form" x-model="availableAtTime"
                                    class="block w-1/2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                                    <x-time-select-options />
                                </select>
                            </div>
                            <input type="hidden" name="available_at" form="update-form" :value="availableAtCombined" />
                        ...
                        <div>
                            ...
                            <select id="assigned_reader_id" name="assigned_reader_id" form="update-form"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                                <option value="">None</option>
                        ...
                        <div>
                            ...
                            <input type="text" id="helpscout_ticket_number" name="helpscout_ticket_number" form="update-form"
                    ...
                    {{-- Upload & Invoice --}}
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 space-y-4">
                        <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Upload</h3>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="date" id="date" name="date" form="update-form"
                            ...
                            <div>
                                ...
                                <input type="time" id="time" name="time" form="update-form"
                        ...
                        <div class="pt-3 border-t border-gray-100">
                            <div class="flex items-center gap-3">
                                <input type="hidden" name="create_invoice" form="update-form" value="0">
                                <input type="checkbox" id="create_invoice" name="create_invoice" form="update-form" value="1"
                            ...
                            <div x-show="createInvoice" x-cloak class="mt-3 space-y-3">
                                <div>
                                    ...
                                    <select id="invoice_client_id" name="invoice_client_id" form="update-form"
                                ...
                                <div>
                                    ...
                                    <div class="mt-1 relative">
                                        ...
                                        <x-text-input id="invoice_amount" name="invoice_amount" form="update-form" type="number"
                                            step="0.01" min="0.01"
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5"
                        ...
                        @if ($assignment->hasCloudScript())
                            ...
                            @endphp
                            <div class="mb-4">
                                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Assignment File</p>
                                <p class="text-sm text-gray-800 font-medium mb-2 break-all">{{ $assignment->drive_script_filename ?? 'script.pdf' }}</p>
                    ...
                    {{-- Post-order Discount Code (admin/editor only) --}}
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                        <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="text" readonly
                                   ...
                                   onclick="this.select()"
                                   class="block w-full text-sm font-mono bg-gray-50 border-gray-300 rounded-md shadow-sm" />
                            <form method="POST" action="{{ route('assignments.regenerateDiscountCode', $assignment) }}"
                                ...
                                @csrf
                                <button type="submit"
                                        class="px-3 py-1.5 text-xs font-medium text-indigo-600 border border-indigo-300 rounded-md hover:bg-indigo-50 transition whitespace-nowrap">
                                    Regenerate
                                </button>
                            </form>
                        </div>
