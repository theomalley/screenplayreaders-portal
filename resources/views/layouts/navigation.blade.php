            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(auth()->user()?->canManageAssignments())
                        ...
                        {{-- Invoicing dropdown --}}
                        @php $invoicingActive = request()->routeIs('invoicing.*'); @endphp
                        <div class="relative flex items-center"
                             x-data="{ invoicingOpen: false }"
                             @mouseenter="invoicingOpen = true"
                             @mouseleave="invoicingOpen = false">
                            <button type="button"
                                class="inline-flex items-center gap-1 px-1 pt-1 border-b-2 text-sm font-medium leading-5 focus:outline-none transition duration-150 ease-in-out {{ $invoicingActive ? 'border-indigo-400 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                                Invoicing
                                <svg class="w-3 h-3 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                </svg>
                            </button>
                            <div x-show="invoicingOpen" x-cloak
                        ...
                        @if(auth()->user()?->isAdmin())
                            ...
                            @php $orderLogActive = request()->routeIs('order-log.*'); @endphp
                            <div class="relative flex items-center"
                                 x-data="{ orderLogOpen: false }"
                                 @mouseenter="orderLogOpen = true"
                                 @mouseleave="orderLogOpen = false">
                                <button type="button"
                                    class="inline-flex items-center gap-1 px-1 pt-1 border-b-2 text-sm font-medium leading-5 focus:outline-none transition duration-150 ease-in-out {{ $orderLogActive ? 'border-indigo-400 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                                    Order Log
                                    <svg class="w-3 h-3 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                    </svg>
                                </button>
                                <div x-show="orderLogOpen" x-cloak
                                    ...
                                    <a href="{{ route('order-log.index') }}"
                                       class="block px-4 py-2 text-sm {{ request()->routeIs('order-log.index') ? 'text-indigo-700 font-semibold bg-indigo-50' : 'text-gray-700 hover:bg-gray-50' }}">
                                        All Orders
                                    </a>
                                    <a href="{{ route('order-log.create') }}"
                                       class="block px-4 py-2 text-sm {{ request()->routeIs('order-log.create') ? 'text-indigo-700 font-semibold bg-indigo-50' : 'text-gray-700 hover:bg-gray-50' }}">
                                        Add Order
                                    </a>
                                </div>
                            </div>
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(auth()->user()?->canManageAssignments())
                ...
                @if(auth()->user()?->isAdmin())
                    ...
                    <x-responsive-nav-link :href="route('order-log.index')" :active="request()->routeIs('order-log.index')">
                        {{ __('All Orders') }}
                    </x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('order-log.create')" :active="request()->routeIs('order-log.create')">
                        {{ __('Add Order') }}
                    </x-responsive-nav-link>
                @endif
