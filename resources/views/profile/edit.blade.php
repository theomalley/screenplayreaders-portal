<x-app-layout>
    ...
    <div class="py-{{ $isSelf ? '12' : '6' }}">
        <div class="{{ $isSelf ? 'max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6' : 'max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6' }}">
        ...
        @else
        {{-- ══════════════════════════════════════════════════
             ADMIN-EDIT LAYOUT (editing another user)
        ══════════════════════════════════════════════════ --}}
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                <form id="update-form" method="POST" action="{{ $updateRoute }}" class="p-6 space-y-5"
                    ...
                    <div x-data="{ previewUrl: '{{ $currentPhotoUrl }}' }">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="relative w-16 h-16 rounded-full {{ $isEditingReader ? 'bg-gray-200 text-gray-500' : 'bg-indigo-100 text-indigo-700' }} flex items-center justify-center text-lg font-mono font-semibold shrink-0 overflow-hidden">
                    ...
                    <div class="pt-4 border-t border-gray-100 space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-400">Visible to admin{{ $isEditingReader ? ' and editor' : '' }} only — not shown to customers.</p>
                            <x-input-error :messages="$errors->get('availability_message')" class="mt-1" />
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="upload_warning" name="upload_warning" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500"
                                      placeholder="Optional — shown to customers on the upload form when they select this {{ $isEditingReader ? 'reader' : 'editor' }}.">{{ old('upload_warning', $profile?->upload_warning) }}</textarea>
                            <p class="mt-1 text-xs text-gray-400">Shown as an orange warning box on the customer upload form. Leave blank for no warning.</p>
                        ...
                        <div>
                            ...
                            <div x-data="{
                                ...
                                <textarea x-ref="ta" id="bio" name="bio" rows="5"
                ...
                <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100">
                    @if (auth()->user()->isAdmin() && !$user->isAdmin())
                        <form method="POST"
                              ...
                              onsubmit="return confirm('Permanently delete this {{ $isEditingReader ? 'reader' : 'editor' }}? This cannot be undone.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ $isEditingReader ? route('readers.index') : route('team.index') }}"
            ...
            {{-- Editor Rates + Commission Config (editors only) --}}
            @if ($user->isEditor())
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-5 py-4 bg-gray-50 border-b border-gray-200">
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Toggle which products earn commission and set a custom fixed amount per occurrence. Leave blank to use this editor's commission rate.</p>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 {{ !$enabled ? 'opacity-50' : '' }}">
                                ...
                                <td class="px-4 py-2 text-center">
                                    <input type="checkbox"
                                        name="commissions[{{ $productId }}][enabled]" value="1"
                                        {{ $enabled ? 'checked' : '' }}
